...
function PresentationEdit() {
  ...
  useEffect(() => {
    if (presentation) {
      ...
      window.history.replaceState(
        null,
        "",
        `/edit/${presentationId}/slide/${currentSlideIndex + 1}`
      );
    }
  ...
  const handleDragMouseDown = (e, elementId) => {
    ...
    const handleMouseMove = (moveEvent) => {
      let deltaX =
        ((moveEvent.clientX - initialMouseX) / slideRef.clientWidth) * 100;
      let deltaY =
        ((moveEvent.clientY - initialMouseY) / slideRef.clientHeight) * 100;

      let newX = Math.max(
        0,
        Math.min(100 - element.width, initialPosition.x + deltaX)
      );
      let newY = Math.max(
        0,
        Math.min(100 - element.height, initialPosition.y + deltaY)
      );

      const updatedElements = presentation.slides[
        currentSlideIndex
      ].elements.map((el) =>
  ...
  const handleResizeMouseDown = (e, elementId, corner) => {
    ...
    const handleMouseMove = (moveEvent) => {
      ...
      if (corner.includes("right")) {
        ...
        newWidth = Math.min(
          100,
          Math.max(1, initialWidth + (deltaX / slideRef.clientWidth) * 100)
        );
      }
      if (corner.includes("left")) {
        ...
        if (newWidth !== initialWidth && newWidth >= 1) {
          newX = Math.max(
            0,
            Math.min(
              initialPosition.x + (deltaX / slideRef.clientWidth) * 100,
              initialPosition.x + initialWidth - 1
            )
          );
        }
      }
      if (corner.includes("bottom")) {
        ...
        newHeight = Math.min(
          100,
          Math.max(1, initialHeight + (deltaY / slideRef.clientHeight) * 100)
        );
      }
      if (corner.includes("top")) {
        ...
        if (newHeight !== initialHeight && newHeight >= 1) {
          newY = Math.max(
            0,
            Math.min(
              initialPosition.y + (deltaY / slideRef.clientHeight) * 100,
              initialPosition.y + initialHeight - 1
            )
          );
        }
      }
        ...
        return {
          ...
          elements: slide.elements.map((el) =>
            el.id === editingElementId
              ? { ...newTextElement, id: editingElementId }
              : el
          ),
        };
      }
      ...
      const updatedSlides = presentation.slides.map((slide, index) => {
        if (index === currentSlideIndex) {
          return {
            ...slide,
            elements: slide.elements.map((el) =>
              el.id === editingVideoElementId
                ? {
                    ...newVideoElement,
                    id: editingVideoElementId,
                    src: embeddedURL,
                  }
                : el
            ),
  ...
  const fetchPresentations = async () => {
    try {
      ...
      const presentationData = presentationsData.find(
  ...
  const handleDeletePresentation = async () => {
    const updatedPresentations = allPresentations.filter(
      (p) => p.id !== presentationId
    );
    try {
      await axios.put("/store", {
  ...
  const handleUpdateCodeElement = () => {
    ...
    setTimeout(() => {
      const codeElement = document.querySelector(
        `pre[data-id="${editingCodeElementId}"] code`
      );
      if (codeElement) {
  ...
  const renderCodeElement = (element) => (
    ...
    >
      <code className={`language-${element.language}`}>{element.code}</code>
    </pre>
  );
  ...
  return (
    <div className="flex flex-row min-h-screen space-x-4 bg-slate-100">
      {/* Sidebar Component */}
      <Sidebar
        ...
        onViewClick={async () => {
          ...
          window.open(
            `/preview/${presentationId}/slide/${currentSlideIndex + 1}`,
            "_blank"
          );
        }}
      ...
      <div className="flex flex-col flex-grow p-2 space-y-4">
        ...
        <div className="flex flex-row items-center justify-between  mt-4">
          ...
          <div className="w-25 h-20 bg-gray-300 rounded flex items-center justify-center cursor-pointer">
            ...
            <HiArrowUpOnSquare
              onClick={() => setShowEditThumbnailModal(true)}
              className=" bottom-1 right-1 text-black w-7 h-7 cursor-pointer hover:scale-125 transition-transform duration-200"
            />
          </div>
          ...
          <div className="flex-1 flex flex-row items-center justify-center space-x-2 pr-10">
            ...
            <AiFillEdit
              ...
              className="text-gray-700 w-8 h-8 cursor-pointer hover:scale-110 transition-transform duration-200"
            />
          </div>
        </div>
        ...
        <div className="flex flex-row flex-grow space-x-4">
          ...
          <div className="flex-grow flex items-center justify-center   rounded-lg">
            {/* show slides */}
            <div
              className="relative w-full max-w-5xl aspect-[16/9] bg-gray-200 flex items-center justify-center rounded-lg"
              style={{
                ...transitionStyle,
                background:
                  presentation.slides[currentSlideIndex].background?.type ===
                  "image"
                ...
                backgroundSize:
                  presentation.slides[currentSlideIndex].background?.type ===
                    "image" || defaultBackground.type === "image"
                    ? "cover"
                    : "initial",
            ...
            >
              {/* Slide Index */}
              <div
                className="absolute bottom-2 left-2 text-lg text-gray-700 w-12 h-12 flex items-center justify-center"
                style={{ zIndex: 1000 }}
              >
                {currentSlideIndex + 1}
              </div>
              ...
              {/* Render elements */}
              {presentation.slides[currentSlideIndex].elements.map(
                (element) => (
                  <div
                    key={element.id}
                    ...
                    {element.type === "text" ? (
                      ...
                      <div
                        ...
                        onDoubleClick={(e) => {
                          ...
                          e.stopPropagation();
                          console.log(
                            `Double-clicked video element with id: ${element.id}`
                          );
                          handleEditVideoElement(element.id);
                        }}
                        onContextMenu={(e) => {
          ...
          <div className="w-1/8 flex flex-col space-y-6">
            <div className="relative group">
              ...
              <HiTrash
                onClick={() =>
                  deleteSlide(presentation.slides[currentSlideIndex].id)
                }
        ...
        <div className="flex flex-row items-center justify-between  p-1 rounded-lg  space-x-2">
          ...
          <div className="flex flex-row items-center space-x-4">
            ...
            <HiChevronLeft
              onClick={
                currentSlideIndex > 0 ? navigateToPreviousSlide : undefined
              }
          ...
          >
            <div className="bg-white p-6 rounded-lg shadow-lg w-80">
              <h3 className="text-xl font-bold mb-4 text-gray-800">
                This is the last slide in your presentation!
              </h3>
              <p className="mb-4 text-gray-600">
                To delete this slide, you should delete the presentation
                instead.
              </p>
              <div className="flex justify-end space-x-2">
                <button
                  onClick={() => setShowErrorModal(false)}
                  className="px-4 py-2 bg-lightGray rounded-lg hover:bg-gray-400"
                >
                  Close
                </button>
              </div>
        ...
        {showEditThumbnailModal && (
          ...
          >
            <div className="bg-white p-6 rounded-lg shadow-lg w-80">
              ...
              <div className="flex justify-end space-x-2">
                <button
                  onClick={() => setShowEditThumbnailModal(false)}
                  className="px-4 py-2 bg-gray-300 rounded-lg hover:bg-gray-400"
                >
                  Cancel
                </button>
          ...
          >
            <div className="bg-white p-6 rounded-lg shadow-lg w-80">
              ...
              <input
                ...
                onChange={(e) =>
                  setNewTextElement({ ...newTextElement, text: e.target.value })
                }
          ...
          >
            <div className="bg-white p-6 rounded-lg shadow-lg w-80">
              <h3 className="text-xl font-bold mb-4">
                {isEditingCode ? "Edit Code Element" : "Add Code Element"}
              </h3>
              <textarea
                rows="5"
                value={newCodeElement.code}
                onChange={(e) =>
                  setNewCodeElement({ ...newCodeElement, code: e.target.value })
                }
              ...
              <div className="flex justify-end space-x-2">
                ...
                <button
                  onClick={
                    isEditingCode
                      ? handleUpdateCodeElement
                      : isEditingElement
                      ? handleUpdateTextElement
                      : handleAddCodeElement
                  }
              ...
              {currentSlideBackground.type === "solid" && (
                <div style={{ marginBottom: "1rem" }}>
                  ...
                  <input
                    type="color"
                    value={
                      currentSlideBackground.value || defaultBackground.value
                    }
                    onChange={(e) =>
                      setCurrentSlideBackground({
                        ...currentSlideBackground,
                        ...
                        onChange={(e) => {
                          ...
                          setCurrentSlideBackground((prev) => {
                            const updatedBackground = {
                              ...
                              value: `linear-gradient(to right, ${newColorFrom}, ${
                                prev.colorTo || "#000000"
                              })`,
                            };
                ...
                <button
                  onClick={() => {
                    ...
                    if (currentSlideBackground.type === "image") {
                      if (!currentSlideBackground.value) {
                        setBackgroundError(
                          "Image URL cannot be empty. Please enter a valid URL."
                        );
                        return;
                      }
